...
# broken link.
# AGENTS.md, .beads/ and .remember/ are exempt: all three are written and
# rewritten by other tools (pact's managed protocol block, bd's workspace
# README, the remember plugin's local scratch files), and front matter added by
# hand would be deleted by the next `pact init`, `bd init`, or remember sync. A
# rule a tool un-applies on every sync is not a rule, it is a fight.
mapfile -t docs < <(find . -name '*.md' \
	-not -path './target/*' -not -path './.git/*' -not -path './wt/*' \
	-not -path './.beads/*' -not -path './.remember/*' -not -name 'AGENTS.md' | sort)
