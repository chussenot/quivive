//! vigil — is the fleet alive, right now?
//!
...
///
/// `VIGIL_NOW` (RFC3339) overrides it. That seam is not a convenience: the whole
/// design rests on a tick being a pure function of (ledger, clock, thresholds)
/// — `docs/adr/0001-stream-first-tile.md` — and a claim of purity that cannot be
/// tested end-to-end is a claim about the library, not about the binary somebody
/// runs. With the clock frozen, two invocations over the same ledger must produce
/// byte-identical output, which is exactly what `scripts/fleet-sim.sh` asserts
/// between a resumed read and a cold one.
///
...
pub fn now() -> anyhow::Result<chrono::DateTime<chrono::Utc>> {
    match std::env::var("VIGIL_NOW") {
        Ok(s) if !s.is_empty() => Ok(chrono::DateTime::parse_from_rfc3339(s.trim())
            .map_err(|e| anyhow::anyhow!("VIGIL_NOW=`{s}` is not RFC3339: {e}"))?
            .to_utc()),
        _ => Ok(chrono::Utc::now()),
    }
}
